...
namespace ShpereNavigation
{
    public class AStarFindPath : MonoBehaviour
    {
        ...
        Vector3[] _vertices { get=>sphereNavData.vertices; }
        nearVertex[] _nearVertices { get=>sphereNavData.nearVertices; }
        int _vertexCount { get=>sphereNavData.vertexCount; }

        public List<Vector3> FindPathVectorOrNull(Vector3 startPos, Vector3 goalPos) {
            uint start_id = GetPositionId(startPos);
            uint goal_id = GetPositionId(goalPos);
            List<Vector3> path = FindPathOrNull(start_id, goal_id);
            return path;
        }
        public List<Vector3> FindPathOrNull(uint start_id, uint goal_id)
        {
            if (start_id == goal_id)
                return null;
            ...
            {
                Vector3 slope1 = (path[i + 1]-path[i]).normalized;
                Vector3 slope2 = (path[i + 2]- path[i]).normalized;
                if (Vector3.Distance(slope1, slope2) > 0.5f)
                {
        ...
        public uint GetPositionId(Vector3 position)
        {
            float minDis;
            uint id = 0;
            if (_vertexCount == 0)
